<?php
	if(!empty($status))
	{
		if($status == "-1")
		{
			$sql = mysql_query(" SELECT id FROM failures WHERE status=0 ");
		}
		else if($status == "1"){
			$sql = mysql_query(" SELECT id FROM failures WHERE status=1");
		}
		else{
			$sql = mysql_query(" SELECT id FROM failures");
		}
		if($sql == false){
		...
		if(!empty($orofos)){	
			$sql = mysql_query(" SELECT id FROM failures WHERE orofos='$orofos' AND id IN ($matches)");
			if($sql == false){
				echo "exoyme problima";
			}
			$rows = mysql_num_rows($sql);
